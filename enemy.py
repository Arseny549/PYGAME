...
def battle_window():
    ...
    monster_images = [pygame.image.load('pechenka1 (1).jpg'),
                      pygame.image.load('pechenka2 (1).jpg'),
                      pygame.image.load('pechenka3 (1).jpg'),
                      pygame.image.load('pechenka4 (1).jpg'),
                      pygame.image.load('pechenka5 (1).jpg'),
                      pygame.image.load('pechenka6 (1).jpg'), ]
    ...
    while run:
            ...
            if event.type == pygame.QUIT:
                ...
                if (nobattle_button_rect.collidepoint(mouse_pos) and pygame.mouse.get_pressed()[0]
                    ...
                    if monster_total <= player_total and attempts < 3:
                        dice_roll_monster = roll_dice_monster()
                        try:
                            pygame.draw.rect(battle_screen, square_color, pygame.Rect((50, 250), (300, 100)))
                            text = font.render("Монстр выбил {}".format(dice_roll_monster), 1, (255, 255, 255))
                        except:
                            pass
                        battle_screen.blit(text, (100, 250))
        ...
        try:
            if raboty == 1:
                resized_monster_image = pygame.transform.smoothscale(monster_images[current_image],
                                                                     (new_width, new_height))
                battle_screen.blit(resized_monster_image.convert_alpha(), (x, y))
        except:
            pass
...
class Monster(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.image = pygame.image.load('pechenka1.jpg')
        self.index = 0
...
def first_level():
    ...
    for _ in range(5):
        ...
        attempts = 0
        while pygame.sprite.spritecollideany(monster, wall_list) and attempts < 300:
            monster.rect.x = random.randint(10, 1260)
            monster.rect.y = random.randint(10, 710)
            attempts += 1
        if attempts == 300:
            print("Ошибка: не удалось разместить монстра")
    ...
    while True:
        ...
        if monser_battle:
            battle_window()
            pygame.display.set_mode((1280, 1720))
            continue
...
def second_level():
    ...
    diamond = Coin(300, 50, 'diamond.png')
    all_sprites.add(diamond)
    coins.add(diamond)
